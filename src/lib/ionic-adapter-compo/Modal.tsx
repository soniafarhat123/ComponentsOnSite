import { IonActionSheet, IonAlert, IonContent, IonHeader, IonModal, IonToast, IonToolbar } from '@ionic/react';
...
export function Modal({children,onCancel,onConfirm,cancelTitle,confirmTitle,title,...props }:MyModal):React.JSX.Element{
    return (
      <IonModal {...props}>
      <IonHeader>
        <IonToolbar>
          <Button slot="start" onClick={onCancel}>{cancelTitle}</Button>
          <Title>{title}</Title>
          <Button slot="end" onClick={onConfirm}>{confirmTitle}</Button>
        </IonToolbar>
      </IonHeader>
